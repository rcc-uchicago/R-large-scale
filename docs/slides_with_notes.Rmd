...
Suppose you need to repeat your allele frequency analysis several data
sets. The `summarize_regmap_mafs.R` script only works for one data
set.  So we create a new script "summarize_mafs.R" that is more
...
*Suppose you had to re-run this allele frequency analysis for 1,000
large data sets. How would you design your R and sbatch scripts to
implement this analysis efficiently?*
...
matrix from the genotypes. (For non-quantitative geneticists, this is
effectively the covariance matrix.) This can be done by computing the
...
multiple CPUs to accelerate the computations. Re-run the kinship
...
+ Steps:
    ...
    2. Assess runtime and memory usage.

    3. Re-run centering & scaling with Rcpp, and assess runtime and
       memory usage.
